...
import '@summitx/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@summitx/v3-periphery/contracts/libraries/Path.sol';
import '@summitx/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@summitx/v3-core/contracts/interfaces/ISummitXV3Pool.sol';
import '@summitx/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISummitXV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISummitXV3Pool pool) {
        pool = ISummitXV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISummitXV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISummitXV3Pool(pool).slot0();
            }
