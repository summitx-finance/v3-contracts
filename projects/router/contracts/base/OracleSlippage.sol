...
import '@muchfi/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@muchfi/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@muchfi/v3-periphery/contracts/libraries/Path.sol';
import '@muchfi/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@muchfi/v3-core/contracts/interfaces/IMuchFiV3Pool.sol';
import '@muchfi/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IMuchFiV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IMuchFiV3Pool pool) {
        pool = IMuchFiV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IMuchFiV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IMuchFiV3Pool(pool).slot0();
            }
