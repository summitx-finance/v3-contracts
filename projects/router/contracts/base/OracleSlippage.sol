...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IFusionXV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IFusionXV3Pool pool) {
        pool = IFusionXV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IFusionXV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IFusionXV3Pool(pool).slot0();
            }
