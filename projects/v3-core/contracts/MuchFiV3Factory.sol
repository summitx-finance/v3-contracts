...
/// @title Canonical MuchFi V3 factory
/// @notice Deploys MuchFi V3 pools and manages ownership and control over pool protocol fees
contract MuchFiV3Factory is IMuchFiV3Factory {
    /***/
    ...
    function createPool(
        ...
        pool = IMuchFiV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMuchFiV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMuchFiV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMuchFiV3Pool(pool).setLmPool(lmPool);
    }
