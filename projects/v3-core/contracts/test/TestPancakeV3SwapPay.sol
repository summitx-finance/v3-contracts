...
contract TestPancakeV3SwapPay is IFusionXV3SwapCallback {
    ...
    function fusionXV3SwapCallback(
        ...
        if (pay0 > 0) {
            IERC20Minimal(IFusionXV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(pay0));
        } else if (pay1 > 0) {
            IERC20Minimal(IFusionXV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(pay1));
        }
    }
}
