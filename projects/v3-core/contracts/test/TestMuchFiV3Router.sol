...
contract TestMuchFiV3Router is IMuchFiV3SwapCallback {
    ...
    function muchfiV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IMuchFiV3Pool(msg.sender).token0()
                : IMuchFiV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IMuchFiV3Pool(pools[0]).token1();
            IMuchFiV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IMuchFiV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IMuchFiV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
