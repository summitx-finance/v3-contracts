...
import '../interfaces/callback/IFusionXV3MintCallback.sol';
...
contract TestPancakeV3Callee is IFusionXV3MintCallback, IFusionXV3SwapCallback, IFusionXV3FlashCallback {
    ...
    function swapToLowerSqrtPrice(
        ...
        IFusionXV3Pool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function swapToHigherSqrtPrice(
        ...
        IFusionXV3Pool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function mint(
        ...
        IFusionXV3Pool(pool).mint(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }
    ...
    function fusionXV3MintCallback(
        ...
        if (amount0Owed > 0)
            IERC20Minimal(IFusionXV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IFusionXV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IFusionXV3Pool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }

    function fusionXV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
        ...
        if (pay0 > 0) IERC20Minimal(IFusionXV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IFusionXV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
