...
import './interfaces/IMuchFiV3PoolDeployer.sol';
import './interfaces/IMuchFiV3Factory.sol';
...
import '@muchfi/v3-lm-pool/contracts/interfaces/IMuchFiV3LmPool.sol';
import './interfaces/IProtocolFeeCollector.sol';

contract MuchFiV3Pool is IMuchFiV3Pool {
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == IMuchFiV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IMuchFiV3PoolDeployer(msg.sender).parameters();
    ...
    function mint(
        ...
        IMuchFiV3MintCallback(msg.sender).muchfiV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IMuchFiV3SwapCallback(msg.sender).muchfiV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        IMuchFiV3FlashCallback(msg.sender).muchfiV3FlashCallback(fee0, fee1, data);
    ...
    function setLmPool(address _lmPool) external override onlyFactoryOrFactoryOwner {
      lmPool = IMuchFiV3LmPool(_lmPool);
      emit SetLmPoolEvent(address(_lmPool));
    }

    /// @dev Auto-collect protocol fees if collector is set and fees are substantial
    /// @dev This reduces the need for manual fee collection and ensures fees flow to the collector
    function _autoCollectProtocolFees() internal {
        address collector = IMuchFiV3Factory(factory).protocolFeeCollector();
        if (collector == address(0)) return;
        
        uint128 amount0 = protocolFees.token0;
        uint128 amount1 = protocolFees.token1;
        
        // Only collect if fees are substantial (> 0.01 token units)
        uint256 threshold = 1e16; // 0.01 tokens with 18 decimals
        if (amount0 < threshold && amount1 < threshold) return;
        
        // Collect fees and send to protocol fee collector
        if (amount0 > 0) {
            protocolFees.token0 = 0;
            TransferHelper.safeTransfer(token0, collector, amount0);
        }
        if (amount1 > 0) {
            protocolFees.token1 = 0;
            TransferHelper.safeTransfer(token1, collector, amount1);
        }
        
        // Trigger the protocol fee collector to distribute fees
        try IProtocolFeeCollector(collector).collectProtocolFees(IMuchFiV3Pool(this)) {} catch {}
    }
}