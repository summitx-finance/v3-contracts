...
import './MuchFiV3Pool.sol';

contract MuchFiV3PoolDeployer is IMuchFiV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(MuchFiV3Pool).creationCode));
    ...
    /// @param factory The contract address of the MuchFi V3 factory
    ...
    function deploy(
        ...
        pool = address(new MuchFiV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}