...
import '@muchfi/v3-core/contracts/interfaces/callback/IMuchFiV3FlashCallback.sol';
...
/// @notice An example contract using the MuchFiV3 flash function
contract PairFlash is IMuchFiV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `MuchFiV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IMuchFiV3Pool pool = IMuchFiV3Pool(PoolAddress.computeAddress(deployer, poolKey));
