...
import '@pancakeswap/v3-core/contracts/interfaces/callback/IFusionXV3FlashCallback.sol';
...
/// @notice An example contract using the FusionXV3 flash function
contract PairFlash is IFusionXV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `FusionXV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IFusionXV3Pool pool = IFusionXV3Pool(PoolAddress.computeAddress(deployer, poolKey));
