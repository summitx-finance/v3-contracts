...
import { IMuchFiV3Pool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IMuchFiV3Pool, IMuchFiV3Pool, IMuchFiV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IMuchFiV3PoolABI, wallet)) as [
      IMuchFiV3Pool,
      IMuchFiV3Pool,
      IMuchFiV3Pool
    ]
  ...
  let pools: [IMuchFiV3Pool, IMuchFiV3Pool, IMuchFiV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestMuchFiV3Callee')
