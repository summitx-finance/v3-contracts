...
contract SummitXERC20 is ISummitXERC20 {
    using SafeMath for uint256;

    string public constant name = "SummitX LPs";
    string public constant symbol = "SummitX-LP";
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "SummitX: EXPIRED");
...
interface ISummitXCallee {
    function SummitXCall(
        address sender,
        uint256 amount0,
        uint256 amount1,
        bytes calldata data
...
contract SummitXPair is ISummitXPair, SummitXERC20 {
    using SafeMath for uint256;
    ...
    modifier lock() {
        require(unlocked == 1, "SummitX: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "SummitX: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        address feeTo = ISummitXFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "SummitX: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "SummitX: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "SummitX: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "SummitX: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, "SummitX: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                ISummitXCallee(to).SummitXCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "SummitX: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
...
contract SummitXFactory is ISummitXFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(SummitXPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB)
        ...
        returns (address pair)
    {
        require(tokenA != tokenB, "SummitX: IDENTICAL_ADDRESSES");
        ...
        ISummitXPair(pair).initialize(token0, token1);
