...
import "./interfaces/IVSummitX.sol";

contract SummitXPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 summitxAtLastUserAction; // keep track of summitx deposited at the last user action.
    ...
    event NewVSummitXContract(address VSummitX);
    ...
    /**
     * @notice Constructor
     * @param _token: SummitX token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: summitx pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(summitxPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the summitx owner address or the operator address.
     */
    modifier onlyOperatorOrSummitXOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or summitx owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onSummitXPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.summitxAtLastUserAction;
    ...
    /**
     * @notice Unlock user summitx funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrSummitXOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the SummitX Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in SUMMITX)
    ...
    function depositOperation(
        ...
        if (VSummitX != address(0)) {
            IVSummitX(VSummitX).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.summitxAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the SummitX Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VSummitX != address(0)) {
            IVSummitX(VSummitX).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.summitxAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.summitxAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending SUMMITX tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingSummitX = masterchefV2.pendingSummitX(summitxPoolPID, address(this));
        if (pendingSummitX > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(summitxPoolPID, 0);
    ...
    /**
     * @notice Set VSummitX Contract address
     * @dev Callable by the contract admin.
     */
    function setVSummitXContract(address _VSummitX) external onlyAdmin {
        require(_VSummitX != address(0), "Cannot be zero address");
        VSummitX = _VSummitX;
        emit NewVSummitXContract(VSummitX);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the SummitX Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingSummitXRewards();
     ...
     * @return Returns total pending summitx rewards
     */
    function calculateTotalPendingSummitXRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingSummitX(summitxPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingSummitXRewards()) * (1e18)) / totalShares);
    }
