...
import "./interfaces/IVMuchFi.sol";

contract MuchFiPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 muchfiAtLastUserAction; // keep track of muchfi deposited at the last user action.
    ...
    event NewVMuchFiContract(address VMuchFi);
    ...
    /**
     * @notice Constructor
     * @param _token: MuchFi token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: muchfi pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(muchfiPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the muchfi owner address or the operator address.
     */
    modifier onlyOperatorOrMuchFiOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or muchfi owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onMuchFiPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.muchfiAtLastUserAction;
    ...
    /**
     * @notice Unlock user muchfi funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrMuchFiOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the MuchFi Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in MUCHFI)
    ...
    function depositOperation(
        ...
        if (VMuchFi != address(0)) {
            IVMuchFi(VMuchFi).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.muchfiAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the MuchFi Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VMuchFi != address(0)) {
            IVMuchFi(VMuchFi).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.muchfiAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.muchfiAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending MUCHFI tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingMuchFi = masterchefV2.pendingMuchFi(muchfiPoolPID, address(this));
        if (pendingMuchFi > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(muchfiPoolPID, 0);
    ...
    /**
     * @notice Set VMuchFi Contract address
     * @dev Callable by the contract admin.
     */
    function setVMuchFiContract(address _VMuchFi) external onlyAdmin {
        require(_VMuchFi != address(0), "Cannot be zero address");
        VMuchFi = _VMuchFi;
        emit NewVMuchFiContract(VMuchFi);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the MuchFi Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingMuchFiRewards();
     ...
     * @return Returns total pending muchfi rewards
     */
    function calculateTotalPendingMuchFiRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingMuchFi(muchfiPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingMuchFiRewards()) * (1e18)) / totalShares);
    }
