...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @param _SUMMITX The SUMMITX token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _SUMMITX, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        SUMMITX = _SUMMITX;
    ...
    /// @notice View function for checking pending SUMMITX rewards.
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (SUMMITX.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    /// @notice Update the given pool's SUMMITX allocation point. Can only be called by the owner.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        ...
        SUMMITX.safeTransferFrom(receiver, address(this), _amount);
    ...
    /// @notice Safe Transfer SUMMITX.
    /// @param _to The SUMMITX receiver address.
    /// @param _amount Transfer SUMMITX amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = SUMMITX.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update summitxAmountBelongToMC
            ...
            SUMMITX.safeTransfer(_to, _amount);
        }
    }
