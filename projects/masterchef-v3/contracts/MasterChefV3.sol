...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @param _MUCHFI The MUCHFI token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _MUCHFI, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        MUCHFI = _MUCHFI;
    ...
    /// @notice View function for checking pending MUCHFI rewards.
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (MUCHFI.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    /// @notice Update the given pool's MUCHFI allocation point. Can only be called by the owner.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        ...
        MUCHFI.safeTransferFrom(receiver, address(this), _amount);
    ...
    /// @notice Safe Transfer MUCHFI.
    /// @param _to The MUCHFI receiver address.
    /// @param _amount Transfer MUCHFI amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = MUCHFI.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update muchfiAmountBelongToMC
            ...
            MUCHFI.safeTransfer(_to, _amount);
        }
    }
