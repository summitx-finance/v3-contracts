...
async function main() {
  ...
  const masterChefV3 = await MasterChefV3.deploy(config.FSX, positionManager_address, config.WNATIVE);

  console.log("masterChefV3 deployed to:", masterChefV3.address);
  // await tryVerify(masterChefV3, [config.FSX, positionManager_address]);
